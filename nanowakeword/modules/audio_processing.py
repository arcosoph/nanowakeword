...
def bulk_predict(
    ...
    # Create NanoWakeWord model objects
    ...
    for chunk in chunks:
        filtered_kwargs = {key: value for key, value in kwargs.items()
                           if key in nanowakeword.Model.__init__.__code__.co_varnames}
        nww = nanowakeword.Model(
...
def download_models(
    ...
    """
    Download the specified models from the release assets in the NanoWakeWord GitHub repository.
    Uses the official urls in the MODELS dictionary in NanoWakeWord/__init__.py.
    ...
    Args:
        ...
        target_directory (str): The directory to save the models to. Defaults to the install location
                                of NanoWakeWord (i.e., the `resources/models` directory).
    Returns:
        None
    """
    ...
    for feature_model in nanowakeword.FEATURE_MODELS.values():
    ...
    for vad_model in nanowakeword.VAD_MODELS.values():
    ...
    # Get all model urls
    official_model_urls = [i["download_url"] for i in nanowakeword.MODELS.values()]
    official_model_names = [i["download_url"].split("/")[-1] for i in nanowakeword.MODELS.values()]
